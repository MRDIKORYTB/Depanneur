...
function addDep(identifier)
    MySQL.Async.execute("INSERT INTO depanneur (`identifier`) VALUES (@identifier)", { ['@identifier'] = identifier })
end


function remDep(identifier)
	MySQL.Async.execute("DELETE FROM depanneur WHERE identifier = '@identifier'", { ['@identifier'] = identifier})
end

function checkIsDep(identifier)
	local result = MySQL.Sync.fetchAll("SELECT * FROM depanneur WHERE identifier = @identifier",
